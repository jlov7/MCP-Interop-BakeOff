...
class RunResult:
    ...
    def to_row(self) -> Dict[str, Any]:
        row = asdict(self)
        row["approvals"] = json.dumps(redact(self.approvals))
        row["metadata"] = json.dumps(redact(self.metadata))
        return row
...
class BaseRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self.artifacts_dir = artifacts_dir or Path(
            os.getenv("USB_AGENTS_ARTIFACT_DIR", ".usb-agents/runs/manual/artifacts")
        )
    ...
    async def _run_task_async(self, task: TaskSpec) -> RunResult:
        ...
        with SpanCapture() as capture:
            with self.tracer.start_as_current_span(
                ...
                try:
                    ...
                    run_span.set_status(Status(status_code=StatusCode.ERROR, description=str(exc)))
        ...
        spans = list(capture.finished_spans())
        ...
        if self.tool_metrics:
            ...
            metadata["tool_durations_ms"] = {
                tool: list(values) for tool, values in self.tool_metrics.items()
            }
    ...
    async def _task_calendar_merge(self, approvals: List[Dict[str, Any]]) -> Dict[str, Any]:
        await self.call_tool("fs_read", {"path": "tasks/calendar.json"}, approvals)
        proposed = [
            {
    ...
    async def _task_http_etl(
        ...
        artifact_path = (
            self.artifacts_dir / f"{self.runtime_name}_{self.transport_mode}_http_etl.csv"
        )
    ...
    async def _task_code_patch(self, approvals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if "def multiply" not in content:
            ...
            patched_source = content.strip() + addition
        approval = self.request_approval(
            "code_exec",
            {"command": "pytest", "source": patched_source},
            approvals,
        )
        if not approval["granted"]:
    ...
    async def call_tool(
        ...
        *,
        ...
        with self.tracer.start_as_current_span(
            ...
            record_tool_latency(
                name, duration_ms, runtime=self.runtime_name, transport=self.transport_mode
            )
        return result
    ...
    def span_to_dict(self, span: Any) -> Dict[str, Any]:
        return {
            ...
            "attributes": redact(dict(span.attributes)),
        }
