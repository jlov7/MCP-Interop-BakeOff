...
try:  # pragma: no cover - optional dependency surface
    ...
    class InMemorySpanExporter(SpanExporter):  # type: ignore[no-redef]
        ...
        def get_finished_spans(self) -> Sequence[Any]:
...
def configure_tracer(
    *,
    ...
    if isinstance(exporter, (ConsoleSpanExporter, InMemorySpanExporter)):
        processor: SimpleSpanProcessor | BatchSpanProcessor = SimpleSpanProcessor(exporter)
    else:
...
def configure_metrics(service_name: str = "usb-agents") -> None:
    ...
    global \
        _METRICS_CONFIGURED, \
        _TOOL_LATENCY_HISTOGRAM, \
        _STDIO_WAIT_HISTOGRAM, \
        _STDIO_ACTIVE_HISTOGRAM, \
        _STDIO_AVAILABLE_HISTOGRAM
...
class SpanCapture:
    ...
    def finished_spans(self) -> Sequence[Any]:
